...
final class NDebugHelpers
{
	...
	public static function renderBlueScreen(Exception $exception)
	{
		if (class_exists('NEnvironment', FALSE)) {
			$application = NEnvironment::getContext()->hasService('Nette\\Application\\Application', TRUE)
				? NEnvironment::getContext()->getService('Nette\\Application\\Application')
				: NULL;
		}
	...
	{
		return strtr(NDebug::$editor, array('%file' => rawurlencode($file), '%line' => $line));
	}
	...
	public static function htmlDump(&$var, $level = 0)
	{
		...
		if (is_bool($var)) {
			...
			if (NDebug::$maxLen && strlen($var) > NDebug::$maxLen) {
				$s = htmlSpecialChars(substr($var, 0, NDebug::$maxLen), ENT_NOQUOTES) . ' ... ';
			} else {
			...
			} elseif ($level < NDebug::$maxDepth || !NDebug::$maxDepth) {
				$s .= "<code>$brackets[0]\n";
				$var[$marker] = $brackets;
				foreach ($var as $k => &$v) {
	...
	public static function jsonDump(&$var, $level = 0)
	{
		if (is_bool($var) || is_null($var) || is_int($var) || is_float($var)) {
			...
			if (NDebug::$maxLen && strlen($var) > NDebug::$maxLen) {
				$var = substr($var, 0, NDebug::$maxLen) . " \xE2\x80\xA6 ";
			}
