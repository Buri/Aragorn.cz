	...
	private function parse($s)
	{
		...
		while ($this->offset < $len) {
			...
			if (!$matches) { // EOF
				...
				if ($code === FALSE) {
					throw new NLatteException("Unknown macro {{$matches['macro']}}", 0, $this->line);
				}
		...
		foreach ($this->tags as $tag) {
			if (!$tag->isMacro && !empty($tag->attrs)) {
				throw new NLatteException("Missing end tag </$tag->name> for macro-attribute " . self::HTML_PREFIX
		...
		if (!$matches || !empty($matches['macro']) || !empty($matches['comment'])) { // EOF or {macro}
			...
			if ($tag->isMacro || !empty($tag->attrs)) {
				if ($tag->isMacro) {
					...
					if ($code === FALSE) {
						throw new NLatteException("Unknown tag-macro <$tag->name>", 0, $this->line);
					}
					if ($isEmpty) {
					...
					if ($code === FALSE) {
						throw new NLatteException("Unknown macro-attribute " . self::HTML_PREFIX
			...
			if (!$tag->closing && (strcasecmp($tag->name, 'script') === 0 || strcasecmp($tag->name, 'style') === 0)) {
				...
				$this->escape = 'NTemplateHelpers::escape' . (strcasecmp($tag->name, 'style') ? 'Js' : 'Css');
			} else {
			...
			if ($isSpecial || $tag->isMacro) {
				...
				$this->escape = strncasecmp($name, 'on', 2)
					? ('NTemplateHelpers::escape' . (strcasecmp($name, 'style') ? 'Html' : 'Css'))
