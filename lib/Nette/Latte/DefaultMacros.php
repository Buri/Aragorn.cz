...
class NLatteMacros extends NObject
{
	...
	public function __construct()
	{
		...
		$this->tokenizer = new NTokenizer(array(
			self::T_WHITESPACE => '\s+',
			self::T_COMMENT => '(?s)/\*.*?\*/',
			NLatteFilter::RE_STRING,
	...
	public function finalize(& $s)
	{
		...
		if (count($this->blocks) === 1) { // auto-close last block
			...
			throw new NLatteException("There are unclosed blocks.", 0, $this->filter->line);
		}
...
?>' . $s;
		...
		$s = "<?php\n"
			. '$_l = NLatteMacros::initRuntime($template, '
	...
	public function macro($macro, $content = '', $modifiers = '')
	{
		if (func_num_args() === 1) {  // {macro val|modifiers}
			list(, $macro, $content, $modifiers) = NString::match(
				$macro,
				'#^(/?[a-z0-9.:]+)?(.*?)(\\|[a-z](?:'.NLatteFilter::RE_STRING.'|[^\'"]+)*)?$()#is'
			);
		...
		if ($closing) {
			...
			if (!$node || "/$node->name" !== $macro
				|| ($content && !NString::startsWith("$node->content ", "$content ")) || $modifiers
			) {
				$macro .= $content ? ' ' : '';
				throw new NLatteException("Unexpected macro {{$macro}{$content}{$modifiers}}"
		...
		return NString::replace(
			$this->macros[$macro],
			'#%(.*?)%#',
			callback(create_function('$m', 'extract(NClosureFix::$vars['.NClosureFix::uses(array('This'=>$This,'node'=> $node)).'], EXTR_REFS);
				if ($m[1]) {
	...
	public function macroSyntax($var)
	{
		...
		default:
			throw new NLatteException("Unknown syntax '$var'", 0, $this->filter->line);
		}
	}
	...
	public function macroInclude($content, $modifiers, $isDefinition = FALSE)
	{
		...
		if ($destination === NULL) {
			throw new NLatteException("Missing destination in {include}", 0, $this->filter->line);

		} elseif ($destination[0] === '#') { // include #block
			$destination = ltrim($destination, '#');
			if (!NString::match($destination, '#^\$?' . self::RE_IDENTIFIER . '$#')) {
				throw new NLatteException("Included block name must be alphanumeric string, '$destination' given.", 0, $this->filter->line);
			}
			...
			if ($destination === 'parent' || $destination === 'this') {
				...
				if (!$item) {
					throw new NLatteException("Cannot include $destination block outside of any block.", 0, $this->filter->line);
				}
				$destination = $item[1];
			}
			...
			$cmd = 'NLatteMacros::includeTemplate(' . $destination . ', '
	...
	{
		if (!$content) {
			throw new NLatteException("Missing destination in {extends}", 0, $this->filter->line);
		}
		if (!empty($this->blocks)) {
			throw new NLatteException("{extends} must be placed outside any block.", 0, $this->filter->line);
		}
		if ($this->extends !== NULL) {
			throw new NLatteException("Multiple {extends} declarations are not allowed.", 0, $this->filter->line);
		}
	...
	public function macroBlock($content, $modifiers)
	{
		...
		if ($name === NULL) { // anonymous block
			...
			if (!NString::match($name, '#^' . self::RE_IDENTIFIER . '$#')) {
				throw new NLatteException("Block name must be alphanumeric string, '$name' given.", 0, $this->filter->line);

			} elseif (isset($this->namedBlocks[$name])) {
				throw new NLatteException("Cannot redeclare block '$name'", 0, $this->filter->line);
			}
	...
	public function macroCapture($content, $modifiers)
	{
		...
		if (substr($name, 0, 1) !== '$') {
			throw new NLatteException("Invalid capture block parameter '$name'", 0, $this->filter->line);
		}
	...
	{
		return '$iterator = $_l->its[] = new NSmartCachingIterator('
	...
	{
		return NString::replace($content . ' ', '#\)\s+#', ')->');
	}
	...
	public function macroContentType($content)
	{
		...
		// temporary solution
		if (strpos($content, '/')) {
			return 'NEnvironment::getHttpResponse()->setHeader("Content-Type", "' . $content . '")';
		}
	}
	...
	public function macroControl($content)
	{
		...
		if ($pair === NULL) {
			throw new NLatteException("Missing control name in {control}", 0, $this->filter->line);
		}
		$pair = explode(':', $pair, 2);
		...
		foreach ($this->parseMacro(ltrim($modifiers, '|')) as $rec) {
			...
			if ($name === self::T_WHITESPACE) {
				...
				if ($name === self::T_SYMBOL) {
					...
					throw new NLatteException("Modifier name must be alphanumeric string, '$token' given.", 0, $this->filter->line);
				}
			} else {
				if ($token === ':' || $token === ',') {
					$var = $var . ', ';
	...
	{
		if ($matches = NString::match($s, '#^((?>'.NLatteFilter::RE_STRING.'|[^\'"\s,]+)+)\s*,?\s*(.*)$#s')) { // token [,] tail
			$s = $matches[2];
			return $matches[1];
		}
		return NULL;
