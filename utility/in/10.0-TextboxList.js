...
var TextboxList = new Class({
  ...
  options: {/*
    ...
    inBetweenEditableBits: true,
		keys: {previous: 'left'/*Event.Keys.left*/, next: 'right'/*Event.Keys.right*/},
	...
	afterInit: function(){
		...
		document.addEvents({
			...
			keydown: function(ev){
				...
				switch (ev.code){
					case 'backspace'/*Event.Keys.backspace*/:
...
TextboxListBit.Editable = new Class({
  ...
  options: {
		...
		stopEnter: true,
		addOnBlur: false,
		addKeys: 'enter'/*Event.Keys.enter*/
  },
    ...
    this.element = new Element('input', {type: 'text', 'class': this.typeprefix + '-input', autocomplete: 'off', value: this.value ? this.value[1] : ''}).inject(this.bit);		
		...
		if (this.options.addKeys || this.options.stopEnter){
			this.element.addEvent('keydown', function(ev){
				if (!this.focused) return;
				if (this.options.stopEnter && ev.code === 'enter'/*Event.Keys.enter*/) ev.stop();
